...
if (is_get()) {
    ...
    if (!$member) {
        ...
        redirect('/admin/admin_member_accounts.php');
    }
...
if (is_post()) {
    ...
    // Save the image on the server (use __DIR__ to get consistent filesystem path)
    $image_path = __DIR__ . '/../user_img/' . $image_name;
    ...
    if ($f) {
        if (!str_starts_with($f->type, 'image/')) {
            ...
            unlink(__DIR__ . '/user_img' . $image); // Delete the old image
             $image = save_photo($f, $_SERVER['DOCUMENT_ROOT'] . '/user_img'); 
        }
    }
