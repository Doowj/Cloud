...
include_once __DIR__ . '/../components/_base.php';
...
if (is_post()) {
    ...
    if ($action === 'add') {
        ...
        if (empty($_err)) {
            move_uploaded_file($image_tmp, __DIR__ . '/../uploaded_img/' . $image);
            ...
            if ($header === ['name', 'author', 'price', 'description', 'stock', 'category_id', 'image']) {
                ...
                while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
                    ...
                    if (count($data) === 7) {
                        ...
                        if (file_exists(__DIR__ . '/../uploaded_img/' . $image_name)) {
        ...
        <div class="box-container">
            ...
            ?>
                <form action="products.php" method="post" class="box" enctype="multipart/form-data">
                    ...
                    <img src="/uploaded_img/<?= encode($product['image']) ?>" alt="<?= encode($product['name']); ?>">
         ...
         <span class="rating-value"><?= number_format($average_rating, 1); ?></span>
                    ...
                    <button type="button" class="detail-btn">
                        <a style="color: #fefefe;" href="/view_review.php?product_id=<?= encode($product['id']); ?>"  >View Reviews</a>
                    </button>
